...

{
	...
	public class UploaderButton extends YUIAdapter {

	//--------------------------------------
	//  Constructor
	//--------------------------------------

		public function UploaderButton()
		{
			super();
		}
		...
		private var buttonSprite:* = new Sprite();
		...
		*/
		...
		private function renderAsButton (buttonSkinSprite:String) : void {
			...
			buttonSprite = new StyleButton(buttonSkinSprite);
			//buttonSkin.load(new URLRequest(buttonSkinSprite));
			var _this:UploaderButton = this;
			
			var initLoader:Function = function (event:Event) : void {
				//buttonSprite.addChild(buttonSkin);
				
				buttonHeight = buttonSprite.height;// /4;
				buttonWidth = buttonSprite.width;
			...
			buttonSprite.addEventListener(Event.COMPLETE, initLoader);
			buttonSprite.addEventListener(Event.RESIZE, this.buttonResize);
			buttonSprite.addEventListener(IOErrorEvent.IO_ERROR, errorLoader);
			
			var parameters = this.stage.root.loaderInfo.parameters;
			if (parameters["buttonText"]) {
				buttonSprite.text = parameters["buttonText"];
			}
			if (parameters["buttonTextStyle"]) {
				buttonSprite.textStyle = parameters["buttonTextStyle"];
			}
			buttonSprite.load();
		...
		protected function buttonResize(event:Event) {
			var newEvent:Object = new Object();
			newEvent.width = event.currentTarget.width;
			newEvent.height = event.currentTarget.getHeight();
			newEvent.type = "resize";
